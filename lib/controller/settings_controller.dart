...
class SettingsController extends GetxController {
  ...
  Future getSettingsData() async {
    await API
        .handleApiRequest(
            request: () =>
                http.get(Uri.parse(API.settings), headers: API.authheader),
            showLoader: false)
        .then(
      (value) async {
        if (value != null) {
          if (value['success'] == "success" || value['success'] == "Success") {
            ...
            AppThemeData.primaryDefault = Color(int.parse(
                model.data!.driverappColor!.replaceFirst("#", "0xff")));
            ...
            Constant.driverLocationUpdateUnit =
                model.data!.driverLocationUpdate!;
            Constant.minimumWithdrawalAmount =
                model.data!.minimumWithdrawalAmount!;
